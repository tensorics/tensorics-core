...
/**
 * allows to hook in different implementations for more complicated mathematical
 * operations.
 * 
 * @author kfuchsbe
 * @param <T>
...
public class ExtendedFieldImpl<T> extends ExplicitFieldImpl<T> implements ExtendedField<T> {
	...
	/**
	 * Returns the instance which provides methods to convert field elements
	 * from and to double values. It is intended that this mechanism can be
	 * replaced later by a re implementation of the unit-calculations which are
	 * fully based on field elements. To avoid heavy usage until then, this
	 * method is marked as deprecated.
	 * 
	 * @return an object providing conversion methods from/to double values
	 * @deprecated because it shall be removed as soon as a better way to handle
	 *             units (based on field elements) is provided
	 */
	@Override
	@Deprecated
