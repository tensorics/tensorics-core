...
public final class BinaryPredicates {
	...
	/**
	 * Negates the given binary predicate. Thus, the
	 * {@link BinaryPredicate#test(Object, Object)} will return the logically
	 * negated value of the original. For example, if the predicate is
	 * {@code '<='}, then this method will transform the predicate
	 * {@code (a <= b) to !(a <= b)}, which is equivalent to {@code (a > b)}.
	 *
	 * @param original
	...
	/**
	 * Inverts the given binary predicate. Thus, if e.g. the predicate is
	 * {@code '<='}, then this method will transform the predicate
	 * {@code (a <= b)} to {@code (a >= b)}.
	 *
	 * @param original
	 *            the original predicate.
	...
	/**
	 * Returns a logical 'and' combination of the two binary predicates. Thus,
	 * the resulting predicate will only be fulfilled as soon as both predicate
	 * test methods return true.
	 *
	 * @param leftPredicate
	 *            the first predicate which shall be part of the and condition
	 * @param rightPredicate
	 *            the second predicate which shall be part of the and condition
	 * @return a predicate which represents a logical 'AND' between the two
	 *         predicates
	 */
