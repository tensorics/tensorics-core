...
public class ImmutableTensor<T> implements Tensor<T>, Serializable {
	...
	ImmutableTensor(Builder<T> builder) {
		...
		this.shape = Shape.viewOf(builder.getDimensions(),
				this.entries.keySet());
	 ...
	 */
	public static final <T> Tensor<T> fromMap(
			Set<? extends Class<?>> dimensions, Map<Position, T> map) {
	...
	private T findValueOrThrow(Position position) {
		...
		if (entry == null) {
			throw new NoSuchElementException("Entry for position '" + position
					+ "' is not contained in this tensor.");
		}
		return entry;
	}
	...
	public static final class Builder<S> extends AbstractTensorBuilder<S> {
		...
		private final ImmutableMap.Builder<Position, S> entries = ImmutableMap
				.builder();
	...
	public String toString() {
		...
		for (Position position : this.shape.positionSet()) {
			if (index < POSITION_TO_DISPLAY
					|| index > totalSize - POSITION_TO_DISPLAY) {
				buffer.append(position + "=(" + get(position) + "), ");
			} else if (index == POSITION_TO_DISPLAY) {
				buffer.append(".. [" + (totalSize - 2 * POSITION_TO_DISPLAY)
						+ " skipped entries] .. , ");
			}
			index++;
		}
		...
		return Coordinates.dimensionsWithoutClassPath(this) + ", Content:{"
				+ buffer + "}";
	}
