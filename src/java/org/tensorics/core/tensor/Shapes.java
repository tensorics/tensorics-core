...
public final class Shapes {
	...
	/**
	 * Creates a shape, containing all the positions that are contained in both
	 * given shapes. This only makes sense, if the dimensions of the two shapes
	 * are compatible. If they are not, then an {@link IllegalArgumentException}
	 * is thrown.
	 * 
	 * @param left
	 *            the first shape from which to take positions
	 * @param right
	 *            the second shape from which to take positions
	 * @return a shape containing all positions, which are contained in both
	 *         given shapes
	 */
	...
	/**
	 * Creates a shape, containing all the positions that are either contained
	 * in the left or the right shape.This only makes sense, if the dimensions
	 * of the two shapes are the same. If they are not, then an
	 ...
	 * @param right
	 *            the second shape from which to take positions
	 * @return a shape containing all positions, which are contained in at least
	...
	/**
	 * Creates a shape, containing all the positions that are contained at least
	 * in one of the given shapes. This only makes sense, if the dimensions of
	 * the shapes are compatible. If they are not, then an
	 * {@link IllegalArgumentException} is thrown. Further, it is required that
	 * at least one element is contained in the iterable.
	 * 
	 * @param shapes
	 *            the shapes for which the union shall be found
	 * @return a shape which represents the union of all the shapes
	 * @throws IllegalArgumentException
	 *             if the shapes are not of the same dimension
	 * @throws NoSuchElementException
	 *             in case the iterable is empty
	 * @throws NullPointerException
	 *             if the given iterable is {@code null}
	 */
	...
	/**
	 * Creates a shape, containing the positions which are contained in each of
	 * the given shapes. This only makes sense, if the dimensions of the shapes
	 * are the same. If they are not, then an {@link IllegalArgumentException}
	 * is thrown. Further, it is required that at least one element is contained
	 * in the iterable.
	 * 
	 * @param shapes
	 *            the shapes for which the intersection shall be found
	 ...
	 * 
	 * @param left
	 *            the first shape for which to look at the dimensions
	 * @param right
	 *            the second shape where to look at the dimensions.
	...
	/**
	 * Constructs a shape that contains positions, which are derived from the
	 * positions of the given shape by stripping (removing) the given
	 * dimensions. The returned shape contains then only unique resulting
	 * position-parts and thus might be much smaller than the original shape.
	 * 
	 * @param shape
	 *            the shape from which to extract the stripped positions
	 * @param dimensionsToStrip
	 *            the dimensions which shall be removed from the positions of
	 *            the shape
	 * @return a shape containing unique positions, resulting from the shapes
	 *         positions, stripping the given dimensions
	 * @throws NullPointerException
	...
	/**
	 * Constructs a shape that contains all positions resulting from the outer
	 * product of the positions of the left shape with those of the right shape.
	 * It is required that the two shapes have no overlap of dimensions (i.e.
	 * none of the dimensions in the left shape are available in the right and
	 * vice versa).
	 * 
	 * @param left
	 *            the left shape to use for the outer product
	 * @param right
	 *            the right shape to use for the outer product
	 * @return a shape containing positions constructed from all combinations of
	 *         the left positions with the right positions.
	 * @throws NullPointerException
	...
	/**
	 * Checks that both the given arguments are not null and throws exceptions
	 * in case they are.
	 * 
	 * @param left
	 *            the left shape to check for non-nullity
	 * @param right
	 *            the right shape to check for non-nullity
	 * @throws NullPointerException
	 *             if one of the arguments is {@code null} Fs
	 */
