...
/**
 * Represents a binary operation on tensorics, respecting the equivalent of
 * Einstein Summation Convention in tensorics representation. The details are as
 * follows:
 * <p>
 * Since every tensor within the tensorics framework, has to have unique classes
 * for the tensor dimensions (equivalent to the 'indizes' in a usual tensor
 * notation), it is only possible to have at maximum one index (dimension) of
 * the same type in a tensor. Such coordinates are considered per default as
 * contravariant coordinates (indizes). To consistently describe a inner product
 * it is necessary to have corresponding covariant indizes (dimensions in
 ...
 * <p>
 * The most common of the type of operations, represented by this class, is the
 * inner tensor product. However, this operation is designed to be a bit more
 * general, it uses two provided operations:
 * <ul>
 * <li>A binary operation on the type of the tensor values, by which the the
 * elements of the two tensors are treated.
 * <li>An operation on iterables of the tensor values, which is used to reduce
 * the tensor dimension, if applicable (see below). NOTE: This operation must be
 * associative, because it is performed in all directions at the same time. (The
 * user of this class has to take care about this, but it is currently not
 * enforced by the class design).
 * </ul>
 * Using these preconditions, this operation applies the following rules:
 * <ol>
 * <li>The usual broadcasting and shaping rules are applied (with a
 * modification, which take care about the correct mapping of co- and
 * contravariant dimensions)
 * <li>The element-wise binary operation is applied on all the elements
 * <li>For each dimension, where there is a covariant version in one tensor and
 * a contravariant version in the other tensor, the reduction operation is
 * applied. If one tensor contains both variants and the other only one, then
 * the order the operation still is uniquely defined and the order of the
 * operands (left, right) does not matter. However, if both tensors contain both
 * variants, then the contravariant (default) dimension of the right tensor is
 * reduced together with the covariant dimension of the left tensor. This is
 * equivalent (and can be memorized like) the usual matrix multiplication.
...
public class InnerTensorOperation<V> implements BinaryOperation<Tensor<V>> {
	...
	public Tensor<V> perform(Tensor<V> left, Tensor<V> right) {
		...
		/*
		 * produce a multimap from positions, consisting of all but the unique
		 * right dimensions to positions.
		 */
