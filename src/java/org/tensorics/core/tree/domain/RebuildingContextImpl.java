...
/**
 * {@link RebuildingContextImpl} implements {@link RebuildingContext} storing
 * the relation from an old node to the new node in a {@link Map} in which the
 * key is the old node.
 * 
 * @author agorzaws
 */
public class RebuildingContextImpl implements RebuildingContext {
	...
	/**
	 * Constructor creating a {@link RebuildingContext} out of an old
	 * {@link RebuildingContext}
	 * 
	 * @param oldContext
	...
	public <T extends Node> T getUpdated(T node) {
		/*
		 * This cast is save, since we only allow to put values of the same type
		 * of the key
		 */
