...
/**
 * The default implementation of an options registry. It is backed by a class to
 * instance map. Use the factory method {@link #of(Collection)} to create a new
 * instance.
 * <p>
...
public final class ImmutableOptionRegistry<T extends Option<T>> implements OptionRegistry<T> {
	...
	/**
	 * creates a new registry which will contain the given options. If the
	 * collection contains options of the same type (marker interface), then the
	 * ones in the order of the iterator of the collection, will override the
	 * earlier ones.
	 * 
	 * @param processingOptions
	 *            the options that will be contained in the registry
	...
	private <T1 extends Option<T1>> ImmutableOptionRegistry(Collection<T> options) {
		/*
		 * we first have to create a mutable map, because the collection might
		 * contain options of the same class, where later ones will override
		 * previous ones. This would not be allowed by the builder of the
		 * immutable map.
		 */
	...
	public <T1 extends T> OptionRegistry<T> with(T1 newOption) {
		...
		/*
		 * The new option is added at the end, because it will then override the
		 * old option, when put to the map
		 */
