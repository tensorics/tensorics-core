...
/**
 * Contains several options of a certain type. The framework can query for a certain type of option by the use of the
 ...
 * parameter of the option registry), so that the return value for {@link #get(Class)} for each interface is uniquely
 * defined.
 * <p>
 * It is highly recommended, that instances of an option registry are immutable and serializable, since they will be
 * passed along with tensoric expressions. To 'change' options for a certain environment, the {@link #with(Option)} can
 ...
 * 
 * @author kfuchsbe
 * @param <T> the upper bound of the options, which this registry handles
 */
public interface OptionRegistry<T extends Option<T>> {
     ...
     * 
     * @param optionType the class of the marker interface for which an instance has to be retrieved
     ...
     * 
     * @param newOption the new option to replace the previously contained with the same marker interface
     * @return a new instance of the option registry, containing also the new option
     */
