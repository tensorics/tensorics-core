...
/**
 * Test that verifies if {@link TensorSupport} is valid for {@link Tensorbacked}
 * objects as well. Analogous tests covers the and {@link QuantityTensorSupport}
 * 
 * @author agorzaws
 */
public class TensorBackedSupportTest {
	...
	public void testTensorOfQVDoubleCalcDividedTensor() {
		MultibeamQOrbit negativeOf = fullTensoricSupport.negativeOfQ(multibeamTensorBackedOfQV);
		MultibeamQOrbit result = fullTensoricSupport.calculateQ(multibeamTensorBackedOfQV)
	...
	private Tensor<QuantifiedValue<Double>> prepareTensorOfQuantifiedValues(double factor) {
		...
		for (int i = 0; i < 10; i++) {
			builder.at(createCoordinates(i))
					.put(ImmutableQuantifiedValue.of(factor * i, JScienceUnit.of(MICRO(METER))));
		}
		return builder.build();
	}
	...
	private Tensor<QuantifiedValue<Double>> prepareQuantifiedTensor(double factor, double error, boolean valid) {
		...
		for (int i = 0; i < 10; i++) {
			...
			QuantifiedValue<Double> entryValue = ImmutableQuantifiedValue.of(factor * i, JScienceUnit.of(MICRO(METER)));
			if (!valid && i % 2 == 0) {
				Optional<Double> newError = Optional.of(error);
				entryValue = ImmutableQuantifiedValue.of(factor * i, JScienceUnit.of(MICRO(METER))).withError(newError)
						.withValidity(false);
			}
