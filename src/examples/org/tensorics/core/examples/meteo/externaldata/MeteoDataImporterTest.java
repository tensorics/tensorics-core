...
public class MeteoDataImporterTest extends TensoricDoubleSupport {
	...
	@Test
	public void testCreateTwoTensorsWithContextAndMergeThem()
			throws InterruptedException {
		Tensor<QuantifiedValue<Double>> importFromPast = FakeMeteoDataImporter
				.importFromNow(null, null);
		Thread.sleep(100);
		Tensor<QuantifiedValue<Double>> importFromPast2 = FakeMeteoDataImporter
				.importFromNow(null, null);

		System.out.println("one tensor (at now) " + importFromPast);
		System.out.println("the other tensor (at now) " + importFromPast2);

		Set<Tensor<QuantifiedValue<Double>>> tensors = ImmutableSet
				.<Tensor<QuantifiedValue<Double>>> of(importFromPast,
						importFromPast2);
	...
	public void testTwoTensorAddition() {
		Tensor<QuantifiedValue<Double>> _3dTensor = FakeMeteoDataImporter
				.importFromPast(null, null, null);
		Tensor<QuantifiedValue<Double>> _2dTensor = FakeMeteoDataImporter
				.importFromNow(null, null);
		Tensor<QuantifiedValue<Double>> sum = calculateQ(_3dTensor).plus(
				_2dTensor);
	...
	public void testAverageTemperatureByTime() {
		Tensor<QuantifiedValue<Double>> importFromPast = FakeMeteoDataImporter
				.importFromPast(null, null, null);
		Tensor<Double> byAveragingIn = from(valuesOf(importFromPast)).reduce(
				Time.class).byAveragingIn(Structures.doubles());

		System.out.println("Averaged over time " + byAveragingIn);
		Tensor<Double> tempAtTime4 = from(valuesOf(importFromPast))
	...
	public void testTensoricShapeAndDifrentPositions() {
		...
		Tensor<QuantifiedValue<Double>> corruptedTensor = FakeMeteoDataImporter
				.importFromPastCorrupted(null, null, null);
		System.out.println("Corrupted shape" + corruptedTensor.shape());
		System.out
				.println("does big tensor covers the corrupted (smaller) one? "
						+ importFromPast.shape()
								.covers(corruptedTensor.shape()));
	...
	public void testMultiplyByScalar() throws Exception {
		Tensor<Double> tensor = valuesOf(FakeMeteoDataImporter.importFromNow(
				null, null));
		...
		Tensor<Double> dividedTensor = calculate(multiplitedTensor)
				.elementDividedByV(10D);
