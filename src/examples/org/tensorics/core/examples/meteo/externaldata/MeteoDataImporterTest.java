...
public class MeteoDataImporterTest extends TensoricDoubleSupport {
	...
	public void testCreateTwoTensorsWithContextAndMergeThem() throws InterruptedException {
		Tensor<QuantifiedValue<Double>> importFromPast = FakeMeteoDataImporter.importFromNow();
		Thread.sleep(100);
		Tensor<QuantifiedValue<Double>> importFromPast2 = FakeMeteoDataImporter.importFromNow();

		System.out.println("one tensor (at now) " + importFromPast);
		System.out.println("the other tensor (at now) " + importFromPast2);

		Set<Tensor<QuantifiedValue<Double>>> tensors = ImmutableSet.<Tensor<QuantifiedValue<Double>>>of(importFromPast,
				importFromPast2);
	...
	public void testTwoTensorAddition() {
		Tensor<QuantifiedValue<Double>> _3dTensor = FakeMeteoDataImporter.importFromPast();
		Tensor<QuantifiedValue<Double>> _2dTensor = FakeMeteoDataImporter.importFromNow();
	...
	public void testAverageTemperatureByTime() {
		Tensor<QuantifiedValue<Double>> importFromPast = FakeMeteoDataImporter.importFromPast();
		Tensor<Double> byAveragingIn = from(QuantityTensors.valuesOf(importFromPast)).reduce(Time.class)
				.byAveragingIn(Structures.doubles());
	...
	public void testTensoricShapeAndDifrentPositions() {
		...
		Tensor<QuantifiedValue<Double>> corruptedTensor = FakeMeteoDataImporter.importFromPastCorrupted();
